...
# from mmdet.models import PAAHead #modified by jojo 20250415
...
def bbox_mapping_back(bboxes, img_shape, flip_horizontal=False, flip_vertical=False):
    """Map bounding boxes back to the original image space."""
    if flip_horizontal:
        bboxes[:, 0::4] = img_shape[1] - bboxes[:, 2::4]
        bboxes[:, 2::4] = img_shape[1] - bboxes[:, 0::4]
    if flip_vertical:
        bboxes[:, 1::4] = img_shape[0] - bboxes[:, 3::4]
        bboxes[:, 3::4] = img_shape[0] - bboxes[:, 1::4]
    return bboxes
def bbox2result(bboxes, labels, num_classes):
    """将检测结果按类别分组（兼容 MMDetection 2.x）"""
    if bboxes.shape[0] == 0:
        return [np.zeros((0, 5), dtype=np.float32) for _ in range(num_classes)]
    else:
        results = []
        for i in range(num_classes):
            results.append(bboxes[labels == i, :])
        return results
from mmengine.structures import InstanceData
def multiclass_nms(bboxes, scores, iou_threshold=0.5):
    # 转换为 InstanceData 格式
    ...
    # 调用内置 nms
    keep_indices = pred_instances.nms(iou_threshold)
...
class PAA_CoLAD_StdMean_COP_Head(PAA_CoLAD_StdMean_Head):
    ...
    def init_weights(self):
        ...
        #modified by jojo 20250411
        from mmengine.model import BaseModule
        BaseModule.init_weights(self)  # 自动应用 init_cfg
