...
def pkd_loss(pred, target):
    pred = norm(pred)
    target = norm(target)
    # return F.cosine_similarity(pred, target)/2
    return F.mse_loss(pred, target, reduction='none') / 2

@weighted_loss
def fbkd_loss(pred, target):
    pred = norm(pred)
    target = norm(target)
    t_attention_mask = torch.mean(torch.abs( target), [1], keepdim=True)
    size = t_attention_mask.size()
    t_attention_mask = t_attention_mask.view( target.size(0), -1)
    t_attention_mask = torch.softmax(t_attention_mask / t, dim=1) * size[-1] * size[-2]
    t_attention_mask = t_attention_mask.view(size)

    s_attention_mask = torch.mean(torch.abs(pred), [1], keepdim=True)
    size = s_attention_mask.size()
    s_attention_mask = s_attention_mask.view(pred[0].size(0), -1)
    s_attention_mask = torch.softmax(s_attention_mask / t, dim=1) * size[-1] * size[-2]
    s_attention_mask = s_attention_mask.view(size)

    c_t_attention_mask = torch.mean(torch.abs(target), [2, 3], keepdim=True)  # 2 x 256 x 1 x1
    c_size = c_t_attention_mask.size()
    c_t_attention_mask = c_t_attention_mask.view(target[0].size(0), -1)  # 2 x 256
    ...
    c_s_attention_mask = torch.mean(torch.abs(pred), [2, 3], keepdim=True)  # 2 x 256 x 1 x1
    c_size = c_s_attention_mask.size()
    c_s_attention_mask = c_s_attention_mask.view(pred.size(0), -1)  # 2 x 256
...
class FBKDLoss(nn.Module):
    ...
    def forward(self,
        ...
        loss = self.loss_weight * fbkd_loss(
